...
public:
	...
	Distance& operator= (float m)  // Оператор перегрузки присваивания для перевода метров в футы и дюймы
	{
		float fltfeet = MTF * m;
		feet = int(fltfeet);
	...
	Distance& operator=(/*const*/ Distance& d) // Оператор перегрузки присваивания для передачи значений от одного объекта к другому
	{
		feet = d.feet;
		inches = d.inches;
		return *this;
...
int main()
{
	...
	dist2 = mtrs; // Оператор перегрузки присваивания для перевода метров в футы и дюймы
	dist2.showdist();

	dist101 = dist2; // Оператор перегрузки присваивания для передачи значений от одного объекта к другому
	cout << "\ndist101 = ";
	dist101.showdist();
